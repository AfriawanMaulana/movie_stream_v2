  ...
  return (
    <div className="py-20 flex flex-col space-y-10">
      ...
      <div className="flex gap-5 pb-4">
        <div className="relative w-full md:h-[75vh] h-[80vh] overflow-hidden">
          ...
          <div className="absolute bottom-4 left-4 flex flex-col md:flex-row gap-5">
            {/* Poster */}
            <div className="w-36 md:w-52 h-48 md:h-72 rounded-xl bg-white/10 animate-pulse flex-shrink-0" />

            <div className="flex flex-col space-y-4">
              {/* Title */}
              <div className="h-10 w-56 md:w-96 rounded bg-white/10 animate-pulse" />
              ...
              <div className="flex gap-2 items-center">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div
                    key={i}
                    className="h-4 w-14 rounded bg-white/10 animate-pulse"
                  />
                ))}
              </div>
      ...
      <div className="px-5 space-y-10">
        ...
        <div className="flex flex-col space-y-6 my-4">
          ...
          <div className="flex flex-wrap gap-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div
                key={i}
                className="h-8 w-20 rounded-sm bg-white/10 animate-pulse"
              />
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="flex flex-col gap-2">
                <div className="w-full h-[100px] md:h-[110px] rounded-md bg-white/10 animate-pulse" />
        ...
        {/* Comments */}
        <div className="flex flex-col w-full mt-20 px-10 items-center space-y-6">
